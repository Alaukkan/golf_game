...
class Map():
    ...
    def __init__(self, game, course):
        ...
        self.game = game
        self.img = load_image(f"sprites/courses/{course['number']}.png")
        self.flag_img = load_image(f"sprites/courses/flag/01.png")
        self.hole_img = load_image(f"sprites/courses/flag/02.png")
        self.green_img = pygame.transform.scale(load_image(f"sprites/courses/greens/{course['number']}.png"), defs.GAME_RESOLUTION)
    ...
    def get_surface(self, x, y):
        surfarray = pygame.surfarray.pixels3d(self.game.display)
        surface_color = tuple(surfarray[int(x), int(y)])
        del surfarray
        # print(defs.COLOR_TO_SURFACE[surface_color])
        return defs.COLOR_TO_SURFACE[surface_color]
    
    def get_green_gradient(self, x, y):
        x = int((-x - self.green[0] + defs.GREEN_CAM_SIZE) / defs.GREEN_CAM_SCALE)
        y = int((-y - self.green[1] + defs.GREEN_CAM_SIZE) / defs.GREEN_CAM_SCALE)
        direction = int(self.green_gradient[y][x][0]) * math.pi / 4
        gradient = math.radians(int(self.green_gradient[y][x][1]) * 10)
        return direction, gradient
    
    def render(self, surf, offset, game_state):
        if math.sqrt((-self.game.player.ball.pos_x - self.green[0])**2 + (-self.game.player.ball.pos_z - self.green[1])**2) < self.green_radius:
            ...
            surf.blit(self.img, (offset[0] + self.img_offset[0], offset[1] + self.img_offset[1]))
